...
# Log the collected data into the database
def log_data():
    for dwell_time in dwell_times:
        KeystrokeLog.objects.create(key="N/A", action="release", rhythm=dwell_time)
    
    for flight_time in flight_times:
        KeystrokeLog.objects.create(key="N/A", action="release", rhythm=flight_time)

    for up_down_time in up_down_times:
        KeystrokeLog.objects.create(key="N/A", action="release", rhythm=up_down_time)

    for click in click_dynamics:
        MouseActionLog.objects.create(action=click['action'], coordinates=str(click['coordinates']), button=click['button'], timestamp=click['timestamp'])

    # Store backspace count
